        ...
        <form action="doWrite" method="POST" onsubmit="return submitWriteForm(this);">
            ...
            <div class="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
                ...
                <div class="form-control mb-6">
                    <label class="label"><span class="label-text font-bold text-lg">리뷰 대상 공연</span></label>
                ...
                <c:if test="${param.type == 'REVIEW'}">
                    <div class="form-control mb-6">
                        <label class="label"><span class="label-text font-bold">평점</span></label>
                ...
                <div class="form-control mb-6">
                    <label class="label"><span class="label-text font-bold">제목</span></label>
                    <input type="text" name="title" class="input input-bordered w-full" placeholder="제목을 입력하세요" />
                </div>

                <div class="form-control mb-8">
                    <label class="label"><span class="label-text font-bold">내용</span></label>
...
<dialog id="choice_modal" class="modal">
  <div class="modal-box max-w-2xl">
    ...
    <div class="form-control mb-4">
        <input type="text" id="searchKeyword" onkeyup="filterList()" placeholder="공연명 검색..." class="input input-bordered w-full" />
    </div>
                ...
                <c:choose>
                    <c:when test="${param.type == 'REVIEW'}">
                        <c:forEach var="res" items="${myReservations}">
                            <tr class="hover cursor-pointer" onclick="selectTarget(${res.concertId}, '${res.extra__concertTitle}', ${res.id})">
                                <td>
                                    <div class="font-bold">${res.extra__concertTitle}</div>
                                    <div class="text-xs text-gray-400">예매일: ${res.regDate} / 좌석: ${res.seatIds}</div>
                                </td>
                                <td class="text-right"><span class="badge badge-primary">선택</span></td>
                            </tr>
                        </c:forEach>
                    </c:when>
                    <c:otherwise>
                        <c:forEach var="concert" items="${allConcerts}">
                            <tr class="hover cursor-pointer" onclick="selectTarget(${concert.id}, '${concert.title}', null)">
                                <td><div class="font-bold">${concert.title}</div></td>
                                <td class="text-right"><span class="badge">선택</span></td>
                            </tr>
                        </c:forEach>
                    </c:otherwise>
                </c:choose>
...
<script>
    ...
    function submitWriteForm(form) {
        if (!form.concertId.value) { alert('리뷰 대상을 선택해주세요.'); return false; }
        if (form.title.value.trim().length == 0) { alert('제목을 입력해주세요.'); return false; }
        if (form.body.value.trim().length == 0) { alert('내용을 입력해주세요.'); return false; }
        return true;
    }
</script>