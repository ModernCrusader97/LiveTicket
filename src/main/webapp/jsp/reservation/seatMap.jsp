...
<%@ page import="LiveTicket.DTO.Seat"%>
<%@ page import="java.util.List"%>
...
<body>
	...
	<div class="seat-container">
		...
		String currentRow = "";
		for (Seat seat : seats) {
			String rowName = (String) seat.getRowName();
			String status = (String) seat.getStatus();
			...
			? "onclick='selectSeat(this, " + seat.getId() + ", " + seat.getVersion() + ", \"" + seat.getGradeName()
					+ "\", " + seat.getPrice() + ")'"
			: "";
		%>
		<div class="seat <%=status%>" <%=clickEvent%> title="<%=seat.getGradeName()%>석 - <%=seat.getPrice() %>원">
			<%=seat.getColNumber()%>
		</div>
		<%
		}
	...
	<script>
		function selectSeat(element, seatId, version, gradeName, price) {
			...
			document.getElementById('selectedInfo').innerText = "선택된 좌석: "
					+ gradeName + "석 " + price + "원";
		}
